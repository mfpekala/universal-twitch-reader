...
class Classifier:
    ...
    def __init__(self, box_dir: str, PCA_components=2):
        ...
        self.key2ix = {}
        print("Loading boxes...")
        for (ix, item) in tqdm(enumerate(self.shape_loader), total=len(self.shape_loader)):
            box = Box(
                id=item["id"][0],
                left=item["left"][0],
                top=item["top"][0],
    ...
    def do_PCA(self):
        ...
        mobilenet_pretrained = models.mobilenet_v2(weights=models.MobileNet_V2_Weights.DEFAULT).to(device) 
        output_list = []
        ...
        for (ix, item) in tqdm(enumerate(self.shape_loader), total=len(self.shape_loader)):
            ...
            features = torch.cat((features, torch.tensor([[item["left"] / 5, item["top"] / 5, item["width"] / 5, item["height"] / 5]])), dim=1)
...
def classify(box_dir: str):
    ...
    labeller.run()
    labeller.plot_user_labeled("User Labeled", classifier.explained_variance_ratios[0], classifier.explained_variance_ratios[1])
    labeller.finalize_labels()
    labeller.plot_all_labelled("All Labelled", classifier.explained_variance_ratios[0], classifier.explained_variance_ratios[1])

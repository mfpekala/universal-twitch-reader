...
class Engine:
    ...
    def solicit_files(self):
        ...
        # Construct clean identifiers we can use for intermediate files
        self.identifiers = [
            text.split("/")[-1].split(".")[0] for text in self.input_files
        ]
        ...
        self.identifiers = [
            clean_identifier(identifier) for identifier in self.identifiers
        ]
        # Check for duplicate identifiers
        assert len(self.identifiers) == len(
            set(self.identifiers)
        ), "Duplicate identifiers found. Make sure each file has a unique name."
    ...
    def _prepare_internal(self):
        ...
        intermediate_dirs = [
            f"data/images/{identifier}" for identifier in self.identifiers
        ]
        intermediate_dirs += [
            f"data/images/{identifier}/frames" for identifier in self.identifiers
        ]
        intermediate_dirs += [
            f"data/boxes/{identifier}" for identifier in self.identifiers
        ]
        intermediate_dirs += [
            f"data/boxes/{identifier}/color" for identifier in self.identifiers
        ]
        intermediate_dirs += [
            f"data/boxes/{identifier}/bw" for identifier in self.identifiers
        ]
    ...
    def do_work(self, ix: int) -> float:
        ...
        duration = convert2frames(
            self.input_files[ix],
            f"data/images/{self.identifiers[ix]}/frames",
        ...
        process(
            f"data/boxes/{self.identifiers[ix]}",
            f"{self.output_dir}/{self.identifiers[ix]}",
            self.classifier,
            self.labeller,
        )
    ...
    def run(self):
        ...
        for ix in range(len(self.input_files)):
            ...
            # Time the work
            start = time.time()
            vid_length = self.do_work(ix)
            end = time.time()
            print_important(
                f"Finished processing video {ix+1} of {len(self.input_files)} in {(end-start):.2f} seconds"
            )
            print_important(f"Real-time factor: {vid_length/(end-start):.2f}")

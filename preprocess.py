...
class Box:
    ...
    def distance(self, other: "Box") -> float:
        ...
        dist_between_centers = abs(
            self.left + self.width / 2 - other.left - other.width / 2
        ) + abs(self.top + self.height / 2 - other.top - other.height / 2)
...
def get_boxes(img, conf_threshold=70, box_merge_dist=25.0):
    ...
    for i in range(len(results["block_num"])):
        ...
        # Ignore big boxes
        if (
            results["width"][i] > img.shape[0] / 2
            or results["height"][i] > img.shape[1] / 2
        ):
            continue
        box = Box(
            results["block_num"][i],
...
def pre_process(
    ...
    should_binarize=True,
    binarize_threshold=125,
    save_as_new_imgs=False,
):
    f_dataset = datasets.ImageFolder(root=inp_dir, transform=transforms.ToTensor())
